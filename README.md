...
在开发环境中，前端通过 Vite 开发服务器运行在 5173 端口，后端直接运行在 8080 端口；在容器化部署环境中，三类服务通过 Docker Compose 进行统一编排，前端由 Nginx 暴露 8080 端口，后端映射为 8081 端口，数据库容器监听 5432 端口。
...
整体来看，统计功能更偏向教学与示例用途，重点展示从访问日志到统计结果的完整处理链路，在保证实现清晰可读的前提下兼顾一定的性能考虑。
...
在端到端测试部分，通过 Playwright 的 `webServer` 配置复用已有的 Vite 开发服务器，避免重复启动导致的 5173 端口占用问题。项目同时提供 `run-tests.sh` 等脚本，对后端测试和前端测试进行统一调度，便于在本地完成一次性验证。
