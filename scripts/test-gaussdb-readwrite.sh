...
if PGPASSWORD=$DB_PASS psql -h $PRIMARY_HOST -p $REPLICA_PORT -U $DB_USER -d $DB_NAME -c "SELECT 1;" > /dev/null 2>&1; then
    echo -e "${GREEN}[OK] 备库连接成功${NC}"
else
    echo -e "${RED}[FAIL] 备库连接失败${NC}"
    exit 1
fi
echo ""
...
if [ "$REPLICA_COUNT" == "3" ]; then
    ...
    echo -e "${YELLOW}[WARN] 备库数据未完全同步 (记录数: $REPLICA_COUNT, 期望: 3)${NC}"
fi
echo ""
...
then
    ...
    if [ "$USER_COUNT" == "1" ]; then
        echo -e "${GREEN}[OK] 业务表数据已同步到备库${NC}"
    else
        echo -e "${YELLOW}[WARN] 业务表数据未同步${NC}"
    fi
else
    echo -e "${RED}[FAIL] 业务表写入失败${NC}"
fi
echo ""
