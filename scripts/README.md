# 部署与测试脚本说明

本目录包含项目的所有部署、验证和测试脚本。

## ⚠️ 重要：端口配置

**当前 GaussDB 集群端口配置：**

- **主库**: 5432 (HA 端口: 5433)
- **备库 1**: 5434 (HA 端口: 5435)
- **备库 2**: 5436 (HA 端口: 5437)

> 注意：GaussDB 每个实例需要 2 个连续端口（主端口+HA 端口），因此使用间隔端口避免冲突。

---

## 脚本分类

### 🔧 GaussDB 集群管理

#### `refactor-ports.sh` ⭐ 推荐首先运行

重构 GaussDB 集群端口配置

**功能**：

- 停止所有实例
- 修改端口配置（5432, 5434, 5436）
- 重新启动所有实例
- 验证端口监听

**使用方法**：

```bash
./scripts/refactor-ports.sh
```

**何时使用**：

- 首次部署后
- 端口冲突时
- 集群重置时

---

#### `verify-gaussdb-cluster.sh`

验证 GaussDB 一主二备集群状态

**功能**：

- 检查主库、备库 1、备库 2 运行状态
- 验证主备复制连接
- 测试主库写入功能
- 测试备库读取功能
- 验证数据同步
- 显示复制延迟统计

**使用方法**：

```bash
./scripts/verify-gaussdb-cluster.sh
```

**前置条件**：

- GaussDB 集群已部署
- 三个实例正在运行（端口 5432, 5434, 5436）

---

#### `test-gaussdb-readwrite.sh`

测试 GaussDB 读写功能

**功能**：

- 测试主库和备库连接
- 测试主库写入
- 测试备库读取
- 验证主备数据一致性
- 测试业务表读写

**使用方法**：

```bash
# 默认配置（端口 5432, 5434）
./scripts/test-gaussdb-readwrite.sh

# 自定义配置
./scripts/test-gaussdb-readwrite.sh <主库IP> <主库端口> <备库端口>
```

**示例**：

```bash
./scripts/test-gaussdb-readwrite.sh 127.0.0.1 5432 5434
```

---

### 🛠️ 故障排查与修复

#### `diagnose-gaussdb-ports.sh`

诊断 GaussDB 端口配置问题

**功能**：

- 检查当前运行的进程和端口
- 检查配置文件中的端口设置
- 分析端口冲突问题
- 提供修复建议

**使用方法**：

```bash
./scripts/diagnose-gaussdb-ports.sh
```

---

#### `fix-primary-replication.sh`

修复主库复制配置

**功能**：

- 检查并创建 replicator 用户
- 配置 pg_hba.conf 复制规则
- 重置 replicator 密码
- 检查复制参数（wal_level, max_wal_senders）

**使用方法**：

```bash
./scripts/fix-primary-replication.sh
```

---

#### `rebuild-standby2.sh`

重建备库 2

**功能**：

- 停止并备份旧的备库 2 数据
- 使用 gs_basebackup 从主库重新同步
- 配置复制连接和端口
- 启动并验证备库 2

**使用方法**：

```bash
./scripts/rebuild-standby2.sh
```

**注意**：会删除备库 2 的所有数据！

---

#### `check-standby2-logs.sh`

检查备库 2 日志和配置

**功能**：

- 检查数据目录状态
- 查看最新错误日志
- 检查关键配置文件
- 验证文件权限

**使用方法**：

```bash
./scripts/check-standby2-logs.sh
```

---

####`fix-backup-permissions.sh`

修复备份文件权限

**功能**：

- 查找所有 .bak 备份文件
- 修复文件权限为 omm:omm
- 可选删除备份文件

**使用方法**：

```bash
./scripts/fix-backup-permissions.sh
```

---

#### `full-reset-cluster.sh`

完全重置集群（旧版，已被 refactor-ports.sh 替代）

**使用方法**：

```bash
./scripts/full-reset-cluster.sh
```

---

### 🚀 Spark 集成相关

#### `test-spark-gaussdb.sh`

测试 Spark 访问 GaussDB 集群

**功能**：

- 编译 Spark 项目
- 运行 Spark 任务
- 测试 Spark 读写 GaussDB
- 验证主备库访问

**使用方法**：

```bash
# 默认配置
./scripts/test-spark-gaussdb.sh

# 自定义 GaussDB 地址
./scripts/test-spark-gaussdb.sh 10.211.55.11 5432
```

**前置条件**：

- 已安装 Spark
- GaussDB 集群正在运行
- analytics 模块已编译

---

### API 验证相关

#### `verify-api-mapping.sh`

验证前后端 API 映射关系

**功能**：

- 检查前端 API 定义
- 验证后端 Controller 接口
- 自动匹配前后端 API
- 生成验证报告

**使用方法**：

```bash
./scripts/verify-api-mapping.sh
```

**验证的 API**：

- 认证 API（登录、注册）
- 用户 API（获取信息、更新）
- 帖子 API（CRUD、列表、时间线）
- 评论 API（CRUD、查询）
- 好友 API（请求、接受、拒绝、列表、搜索）
- 点赞 API（点赞、取消、检查、计数）
- 上传 API（图片上传）
- 统计 API（Spark 分析、数据查询）

---

### 虚拟机服务管理

#### `start-vm-services.sh`

启动虚拟机上的所有服务

**功能**：

- 启动 GaussDB 主库和备库（如果配置了集群）
- 启动 Docker Compose 服务（后端、前端）
- 执行健康检查
- 显示服务访问地址

**使用方法**：

```bash
./scripts/start-vm-services.sh
```

**自动检测**：

- 自动检测是否为集群模式
- 自动跳过已运行的服务
- 自动等待服务就绪

---

#### `stop-vm-services.sh`

停止虚拟机上的所有服务

**功能**：

- 停止 Docker Compose 服务
- 停止 GaussDB 集群（主库和备库）
- 验证服务已完全停止

**使用方法**：

```bash
./scripts/stop-vm-services.sh
```

---

#### `restart-vm-services.sh`

重启虚拟机上的所有服务

**功能**：

- 依次调用停止和启动脚本
- 确保完全清理后再启动

**使用方法**：

```bash
./scripts/restart-vm-services.sh
```

---

#### `check-vm-status.sh`

检查虚拟机服务状态

**功能**：

- 检查虚拟机连接
- 检查 GaussDB 主库和备库状态
- 检查复制状态
- 检查 Docker 容器状态
- 检查服务端口监听
- 应用健康检查
- 系统资源使用情况

**使用方法**：

```bash
./scripts/check-vm-status.sh
```

**输出信息**：

- GaussDB 版本和连接数
- 复制延迟和状态
- Docker 容器运行状态
- 服务端口监听情况
- CPU、内存、磁盘使用率

---

### 服务启动相关

#### `start-backend.sh`

启动后端服务（本地）

**功能**：

- 编译后端项目
- 启动 Spring Boot 应用
- 支持开发和生产模式

**使用方法**：

```bash
# 开发模式（默认）
./scripts/start-backend.sh

# 生产模式（使用 GaussDB 集群配置）
./scripts/start-backend.sh prod

# 自定义端口
./scripts/start-backend.sh dev 8081
```

**配置说明**：

- `dev` 模式：使用 `application.yml`
- `prod` 或 `gaussdb-cluster` 模式：使用 `application-gaussdb-cluster.yml`

---

#### `start-frontend.sh`

启动前端服务

**功能**：

- 安装前端依赖
- 启动开发服务器
- 构建生产版本

**使用方法**：

```bash
# 开发模式（默认）
./scripts/start-frontend.sh

# 构建生产版本
./scripts/start-frontend.sh build

# 自定义端口
./scripts/start-frontend.sh dev 3000
```

---

### 端到端测试

#### `test-e2e.sh`

完整的端到端功能测试

**功能**：

- 健康检查
- 用户注册和登录
- 获取用户信息
- 创建和查询帖子
- 点赞功能
- 评论功能
- 统计数据查询

**使用方法**：

```bash
# 测试本地服务
./scripts/test-e2e.sh

# 测试远程服务
./scripts/test-e2e.sh http://10.211.55.11:8080
```

**测试流程**：

1. 健康检查
2. 注册新用户
3. 用户登录获取 Token
4. 获取当前用户信息
5. 创建测试帖子
6. 获取帖子列表和详情
7. 点赞帖子
8. 创建评论
9. 获取评论列表
10. 查询统计数据

---

## 📋 完整部署流程

### 方案 A：快速部署（推荐）

#### 步骤 1：同步脚本到虚拟机（在本地 Mac 上）

```bash
cd /Users/lifulin/Desktop/CloudCom
./sync-scripts-to-vm.sh
```

#### 步骤 2：重构端口配置（在虚拟机上）

```bash
ssh root@10.211.55.11
cd CloudCom
./scripts/refactor-ports.sh
```

#### 步骤 3：验证集群（在虚拟机上）

```bash
./scripts/verify-gaussdb-cluster.sh
```

#### 步骤 4：测试读写功能（在虚拟机上）

```bash
./scripts/test-gaussdb-readwrite.sh
```

---

### 方案 B：从头部署

#### 步骤 1：初始化 GaussDB 集群（在虚拟机上）

```bash
cd /Users/lifulin/Desktop/CloudCom  # 或 ~/CloudCom
./setup-gaussdb-single-vm-cluster.sh
```

#### 步骤 2：重构端口配置

```bash
./scripts/refactor-ports.sh
```

#### 步骤 3：修复主库复制配置

```bash
./scripts/fix-primary-replication.sh
```

#### 步骤 4：重建备库 2（如果需要）

```bash
./scripts/rebuild-standby2.sh
```

#### 步骤 5：验证集群

```bash
./scripts/verify-gaussdb-cluster.sh
```

---

### 应用部署

#### 1. 启动后端服务（本地或虚拟机）

```bash
# 本地开发模式
./scripts/start-backend.sh dev

# 使用 GaussDB 集群配置
./scripts/start-backend.sh gaussdb-cluster
```

#### 2. 启动前端服务

```bash
./scripts/start-frontend.sh dev
```

#### 3. 运行测试

```bash
# API 映射验证
./scripts/verify-api-mapping.sh

# 端到端测试
./scripts/test-e2e.sh

# Spark 测试（可选）
./scripts/test-spark-gaussdb.sh
```

---

## Docker 部署

使用项目根目录的部署脚本：

```bash
# 本地 Docker Compose 部署
./deploy.sh local

# 虚拟机部署
./deploy.sh vm

# GaussDB 集群部署
./deploy.sh cluster
```

---

## 🔍 常见问题与故障排查

### 端口冲突问题

**症状**：

- 备库启动失败，提示 "port already in use"
- 一个进程监听多个端口

**解决方法**：

```bash
# 1. 诊断问题
./scripts/diagnose-gaussdb-ports.sh

# 2. 重构端口配置
./scripts/refactor-ports.sh

# 3. 验证结果
./scripts/verify-gaussdb-cluster.sh
```

---

### 主备复制问题

**症状**：

- 备库无法连接到主库
- 复制状态显示为空

**解决方法**：

```bash
# 1. 修复主库复制配置
./scripts/fix-primary-replication.sh

# 2. 重建备库（如果必要）
./scripts/rebuild-standby2.sh

# 3. 验证复制状态
su - omm -c "gsql -d postgres -p 5432 -c 'SELECT * FROM pg_stat_replication;'"
```

---

### 备库启动失败

**症状**：

- 备库 2 一直启动失败
- 日志显示 "could not connect to server"

**解决方法**：

```bash
# 1. 检查日志
./scripts/check-standby2-logs.sh

# 2. 检查主库配置
./scripts/fix-primary-replication.sh

# 3. 重建备库2
./scripts/rebuild-standby2.sh
```

---

### 文件权限问题

**症状**：

- gs_basebackup 失败："Permission denied"
- 无法读取 .bak 文件

**解决方法**：

```bash
./scripts/fix-backup-permissions.sh
```

选择 `y` 删除备份文件

---

### API 测试失败

**问题**：无法获取 Token

**解决方法**：

1. 确保后端服务正在运行
2. 检查数据库连接
3. 验证用户是否存在

**问题**：API 返回 500 错误

**解决方法**：

1. 查看后端日志
2. 检查数据库表结构
3. 验证 SQL 语法兼容性

---

## 💻 环境要求

### GaussDB 集群

- **版本**: openGauss 5.0.0 或 9.2.4
- **用户**: omm
- **端口**:
  - 主库: 5432 (HA: 5433)
  - 备库 1: 5434 (HA: 5435)
  - 备库 2: 5436 (HA: 5437)
- **数据库**: blog_db
- **用户**: bloguser / 747599qw@

### 后端

- JDK 17
- Maven 3.8+
- Spring Boot 3.1.5

### 前端

- Node.js 18+
- npm 或 yarn
- Vite

### Spark（可选）

- Apache Spark 3.5.0
- Scala 2.12

### 虚拟机

- IP: 10.211.55.11
- 用户: root / 747599qw@
- 系统: openEuler 22.03
- SSH: 需要 sshpass

---

## 相关文档

- [项目 README](../README.md)
- [实验报告](../实验报告.md)
- [部署指南](../DEPLOYMENT_GUIDE.md)
