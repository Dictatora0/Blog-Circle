...
if echo "$GAUSSDB_STATUS" | grep -q "running"; then
    echo -e "${GREEN}[OK] GaussDB 已在运行${NC}"
else
    echo "启动 GaussDB..."
    vm_cmd "su - omm -c 'gs_ctl start -D /usr/local/opengauss/data'"
    sleep 5
    echo -e "${GREEN}[OK] GaussDB 启动完成${NC}"
fi
echo ""
...
if curl -s "http://${VM_IP}:8081/actuator/health" | grep -q "UP"; then
    echo -e "${GREEN}[OK] 后端服务正常${NC}"
else
    echo -e "${YELLOW}[WARN] 后端服务可能未就绪，请稍后重试${NC}"
fi

echo -n "检查前端服务: "
if curl -s "http://${VM_IP}:8080" > /dev/null 2>&1; then
    echo -e "${GREEN}[OK] 前端服务正常${NC}"
else
    echo -e "${YELLOW}[WARN] 前端服务可能未就绪，请稍后重试${NC}"
fi
echo ""
