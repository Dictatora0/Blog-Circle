...
if ! docker-compose -f "${LOCAL_COMPOSE_FILE}" ps | grep -q "Up"; then
...
echo "  • 停止前端服务..."
docker-compose -f "${LOCAL_COMPOSE_FILE}" stop frontend

echo "  • 停止后端服务..."
docker-compose -f "${LOCAL_COMPOSE_FILE}" stop backend

echo "  • 停止数据库..."
docker-compose -f "${LOCAL_COMPOSE_FILE}" stop db
...
echo "  • 完全清理（包括数据）：${RED}docker-compose -f ${LOCAL_COMPOSE_FILE} down -v${NC}"
echo ""
